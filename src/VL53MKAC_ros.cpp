...
VL53MKAC_ros_node::VL53MKAC_ros_node()
: scan_seq_(0)
...
{
  // parameters
  ros::param::get("/vl53mkac/scan_range_min", scan_range_min_);
  ros::param::get("/vl53mkac/scan_range_max", scan_range_max_);

  ros::param::get("/vl53mkac/angle_min", angle_min_);
  ros::param::get("/vl53mkac/angle_max", angle_max_);

  ros::param::get("/vl53mkac/frame", frame_id_);

  ros::param::get("/vl53mkac/host_ip", host_ip_);
  ros::param::get("/vl53mkac/port", port_);
  ...
  // publishers
  laser_cloud_pub_ = node_.advertise<sensor_msgs::LaserScan>("/scan", 1);

  // Services
  //connect_laser_service_ = node_.advertiseService("/vl53mkac/connect_laser_srv", &VL53MKAC_ros_node::connect_laser, this);
  // disconnect_laser_service_ = node_.advertiseService("/vl53mkac/disconnect_laser_srv", &VL53MKAC_ros_node::disconnect_laser, this);
  // start_laser_service_ = node_.advertiseService("/vl53mkac/start_laser_srv", &VL53MKAC_ros_node::start_scan_msg, this);
  // stop_laser_service_ = node_.advertiseService("/vl53mkac/stop_laser_srv", &VL53MKAC_ros_node::stop_scan_msg, this);
...
void VL53MKAC_ros_node::initialize_scan_param()
{
...
// Connect laser.
bool VL53MKAC_ros_node::connect()
{
...
// Get Laser config.
bool VL53MKAC_ros_node::getConfig()
{
  return laser.getConfig();
}

// Start continous laser scan msgs
void VL53MKAC_ros_node::startMesure()
{
  laser.startMeas();
}

// Send Heart Beat to laser.
void VL53MKAC_ros_node::sendHB2Lidar()
{
  laser.sendHB();
}

// service: Connect laser.
/*bool VL53MKAC_ros_node::connect_laser(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& resp)
{
    resp.success = true;
...
bool VL53MKAC_ros_node::disconnect_laser(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& resp)
{
    resp.success = true;
    resp.message = "Disconnect Laser.";
    laser.disconnect();
...
// service: start laser.
bool VL53MKAC_ros_node::start_scan_msg(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& resp)
{
    resp.success = true;
...
// service: Stop laser.
bool VL53MKAC_ros_node::stop_scan_msg(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& resp)
{
    resp.success = true;
    ...
    if (laser.packetDecodeExt(&uld_config_))
    {
      ...
      while (laser.GetALim(&uld_config_, &scan_msg_))
      {
        if (laser.initializedLaserConfig())
        {
          ...
          laser_cloud_pub_.publish(scan_msg_);
        }
      }
    }
  }
...
{
  ros::init(argc, argv, "VL53MKAC_ros_node");
  VL53MKAC_ros_node node;
